 ...
 public function save(Request $req, $id = null)
{
    ...
    if (auth()->user()->hasRole('super administrator')) {
        $validationRules['location_id'] = 'required';
    }
    ...
    //photo
    if($req->hasFile('avatar') && !empty($req->avatar) ){
        $userData['photo'] = uploadFile($req->avatar,'users/photos','user'.'-'.time().'-'.rand(000000,999999));
    }
    ...
    if (is_null($id)) {
        ...
        if (auth()->user()->hasRole('superadministrator')) {
            $assignRole = Role::where('name', 'company')->first();
        } elseif (auth()->user()->hasRole('company')) {
            $assignRole = Role::where('name', 'user')->first();
        }
        if ($assignRole) {
            $user->assignRole($assignRole);
        }
        $user->givePermissionTo($req->permissions);
