...
class DashboardController extends Controller
{
    ...
    public function dashboard(Request $req)
    {
        ...
        if (Auth::user()->hasRole('superadministrator')) {
            ...
            $roles = Role::all();
            $dbusers = User::role('company')->latest()->limit(5)->get();
            $properties = Property::with('images', 'location')->latest()->limit(5)->get()->toArray();
            // dd($dbproperties);
    ...
    {set_time_limit(0);
        ...
        $cov_stats = $this->conversionStats($users, $request, $all_contacts);
        $conversionhtml = view('components.conversion_card', compact('cov_stats'))->render();
        $channelsource=$this->getChannelSources($request,allcontacts: $all_contacts);

        if (is_array($channelsource)) {
            $channelsource; // This should work fine as it's an array
            // Check if 'source_channel_modals' key exists
            if (isset($channelsource['source_channel_modals'])) {
                $scm = $channelsource['source_channel_modals'];
            } else {
                // Handle the case where 'source_channel_modals' doesn't exist
                $scm = null; // Or handle accordingly
            }
            if (isset($channelsource['source_channels'])) {
                $sc = $channelsource['source_channels'];
            } else {
                // Handle the case where 'source_channel_modals' doesn't exist
                $sc = null; // Or handle accordingly
            }
        } else {
            // Handle the case where $channelsource is not an array
            $sc= null;
            $scm = null; // Or handle accordingly
        }
    ...
    public function conversionStats($users, $req, $contacts = null)
    {
        ...
        $calls = [
            'total' => $callsCount,
            'conversion_rate' => $contactsCount > 0 ? ($callsCount / $contactsCount) * 100 : 0,
            'direction' => 'up',
            'direction_value' => 0
        ];
        ...
        $sale = [
            'total' => $saleTotal,
            'conversion_rate' => $contactsCount > 0 ? ($saleWon / $contactsCount) * 100 : 0,
            'direction' => 'down',
            'direction_value' => 0
        ];
        ...
        $cov_stats = collect($meetingTypes)->map(function ($groupIds, $key) use ($calendars, $appointments, $contactsCount) {
            ...
            return [
                'total' => $total,
                'conversion_rate' => $contactsCount > 0 ? ($total / $contactsCount) * 100 : 0,
    ...
    */
    ...
    public function getChannelSources(Request $req,$allcontacts=[]){
        $location_id = $req->has('location_id') && !empty($req->location_id)
        ? $req->location_id
        : auth()->user()->location_id;
        // Initialize main sources with default values
        $mainSources = [
            'Facebook' => ['source' => 'Facebook', 'count' => 0, 'total_value' => 0, 'percentage'=>0, 'details' => []],
            'Instagram' => ['source' => 'Instagram', 'count' => 0, 'total_value' => 0,'percentage'=>0, 'details' => []],
            'Youtube' => ['source' => 'Youtube', 'count' => 0, 'total_value' => 0,'percentage'=>0, 'details' => []],
            'Google' => ['source' => 'Google', 'count' => 0, 'total_value' => 0,'percentage'=>0, 'details' => []],
            'Eigen Network' => ['source' => 'Eigen Network', 'count' => 0, 'total_value' => 0, 'percentage'=>0,'details' => []],
        ];
        // Fetch contacts with opportunities


        $contacts = Contact::with('opportunities')
        ->where('location_id', $location_id);
        if ($req->has('user') && !empty($req->user)) {
            $assigned_to = $req->user;
            $contacts->where('assigned_to', $assigned_to);
        }
         if ($req->has('dateRange') && !empty($req->dateRange)) {
            $date_range = explode('-', $req->dateRange);
            $start_date = Carbon::parse(trim($date_range[0]))->format('Y-m-d');
            $end_date = Carbon::parse(trim($date_range[1]))->format('Y-m-d');
            $contacts->whereBetween('date_added',[$start_date,$end_date]);
        }
        $contacts=$contacts->get()->toArray();

        // Process contacts and map the sources based on attributions
        foreach ($contacts as $contact) {
            if (!empty($req['source'])) {
                $source = $req->source;
            }else{
                ...
                // Decode attributions (assuming it's stored as JSON) and ensure it's valid
                $attributions = $contact['attributions'];//json_decode($contact['attributions'], true);
                $attributions = (isset($attributions) && is_array($attributions) && !empty($attributions)) ? $attributions : [];
                // Process the first attribution if available
            if (!empty($attributions)) {
                $attribution = $attributions[0];
                // Check if 'utmSessionSource' and 'medium' exist before proceeding
                if (isset($attribution['utmSessionSource']) && isset($attribution['medium'])) {
                    // Check if 'utmSessionSource' contains 'Paid Social'
                    if (stripos($attribution['utmSessionSource'], 'Paid Social') !== false || stripos($attribution['utmSessionSource'], 'Social Media') !== false) {
                        if (stripos($attribution['medium'], 'facebook') !== false) {
                                $source = 'Facebook';
                            } elseif (stripos($attribution['medium'], 'instagram') !== false) {
                                $source = 'Instagram';
                            } elseif (stripos($attribution['medium'], 'youtube') !== false) {
                                $source = 'Youtube';
                            }
                    } elseif (stripos($attribution['utmSessionSource'], 'Google') !== false && stripos($attribution['medium'], 'form') !== false) {
                            $source = 'Eigen Network';
                    }
                }
            }

                // Aggregate the data into mainSources
                if (!empty($source) && isset($mainSources[$source])) {
                $mainSources[$source]['count'] += 1;
                // Sum won opportunities' monetary value
                $wonOpportunities = array_filter($contact['opportunities'] ??[], function($opportunity) {
                return $opportunity['status'] === 'won' && !is_null($opportunity['monetary_value']);
                });
                $totalValue = array_sum(array_column($wonOpportunities, 'monetary_value'));
                // Add total value of won opportunities to the source
                $mainSources[$source]['total_value'] += $totalValue;
                $mainSources[$source]['details'][] = $contact; // Store details for later use
            }
        }
        $finalOpportunities = $mainSources;
        $source_channels = view('components.channels.channel_source', compact('finalOpportunities'))->render();
        $source_channel_modals = view('components.channels.channel_modals', compact('finalOpportunities'))->render();
        $response = [
            'status' => 'success',
            'source_channels' => $source_channels,
            'source_channel_modals' => $source_channel_modals,
        ];
